...
contract CeVault is IVault, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(string memory name, address ceTokenAddress, address aBNBcAddress) external initializer {
        __Ownable_init();
        __Pausable_init();
    ...
    // deposit
    function depositFor(address recipient, uint256 amount) external override nonReentrant onlyRouter returns (uint256) {
    ...
    // withdraw
    function withdraw(address recipient, uint256 amount) external override nonReentrant returns (uint256) {
    ...
    function _withdraw(address owner, address recipient, uint256 amount) private returns (uint256) {
        uint256 ratio = _aBNBc.ratio();
    ...
    function getPrincipalOf(address account) external view override returns (uint256) {
        uint256 ratio = _aBNBc.ratio();
    ...
    function getYieldFor(address account) external view override returns (uint256) {
